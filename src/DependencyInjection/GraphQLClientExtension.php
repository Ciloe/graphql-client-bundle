...
class GraphQLClientExtension extends Extension
{
    /**
     * {@inheritdoc}
     *
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        $container->getDefinition('graphql_client_bundle.model')
            ->setArgument(0, $config['api']['host'])
            ->setArgument(1, $config['api']['uri'])
            ->setArgument(2, $config['api']['token']);

        if (!empty($config['sources'])) {
            $container->getDefinition('graphql_client_bundle.cache')
                ->setArgument(2, $config['sources']['paths'])
                ->setArgument(3, $config['sources']['extension']);
        } else {
            $container->removeDefinition('graphql_client_bundle.adapter');
            $container->removeDefinition('graphql_client_bundle.cache');
            $container->removeDefinition('graphql_client_bundle.warmer');
        }

        $container->getDefinition('graphql_client_bundle.logger')
